...
Process build(output::Connection, Messenger& self, Process callable, const Address& destination) {
...
namespace message {
    namespace local {
        ...
        Process
        decorate(output::Connection, Process&& upstream, Messenger& self, const Address& destination) {
            return [native = self.native<int>(), upstream = std::move(upstream), destination] {
                struct sockaddr_un addr;
                std::memset(&addr, 0, sizeof(addr));
                // connect to destination
                addr.sun_family  = PF_LOCAL;
